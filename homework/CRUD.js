...
function taidulieubang() {
  var table_sp = `<tr>  
                        <th>STT</th>
                        <th>ID</th>
                        <th><button onclick="sapxep_ten()">Tên Sản Phẩm</button></th>
                        <th><button onclick="sapxep_soluong()">Số Lượng</button></th>
                        <th><button onclick="sapxep_giaban()">Giá Bán</button></th>
                        <th>Ngày Sản Xuất</th>
                        <th>Màu Sắc</th>
                        <th>Tính Năng</th>
                        <th>Thay Đổi</th>
  ...
  document.getElementById("sosp").innerText = `Số Sản Phẩm : ${bangdulieu.length}`;
...
function kiemtratrungdulieu() {
  ...
  for (var i = 0; i < bangdulieu.length; i++) {
    if (
      bangdulieu[i].Ten.trim().toLowerCase() ===
        ten_sp.value.trim().toLowerCase() ||
      bangdulieu[i].ID == id_sp.value
    ) {
      trungdulieu = true;
      break;
    }
...
// Sắp xếp dữ liệu theo số lượng
function sapxep_soluong() {
  bangdulieu.sort(function (sp1, sp2) {
    let a = sp1.SoLuong;
    let b = sp2.SoLuong;
    return a - b;
...
// Sắp xếp theo giá bán từ thấp đến cao
function sapxep_giaban() {
  bangdulieu.sort(function (sp1, sp2) {
    let a = sp1.Gia;
    let b = sp2.Gia;
    return a - b;
